...
action :create do
  ...
  Chef.run_context.resource_collection.find(iptables: new_resource.firewall_name)

  ruby_block "queue iptables rebuild #{new_resource.firewall_name} #{new_resource.name}" do
    block {}
    action :run
    notifies :rebuild, "iptables[#{new_resource.firewall_name}]", :delayed
  end
end
