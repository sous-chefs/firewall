...
class Chef
  class Provider::FirewallRuleFirewalld < Provider
    ...
    def apply_rule(type=nil)
      #firewall_command = "firewall-cmd --direct ipv4 filter"
      firewall_command = "firewall-cmd --direct"

      firewall_rule = ""
      if @new_resource.direction
        firewall_rule << "#{CHAIN[@new_resource.direction]} #{@new_resource.position ? @new_resource.position : 1}"
      else
      ...
      firewall_rule << "-m tcp " if @new_resource.protocol == 'tcp'
      firewall_rule << "-m multiport --sports #{@new_resource.port} " if @new_resource.port
      firewall_rule << "-m multiport --dports #{@new_resource.dest_port} " if @new_resource.dest_port
    ...
    def rule_exists?(rule)
      ...
      cmdstr = "firewall-cmd --direct --get-all-rules | grep -q -- '#{rule}'"
