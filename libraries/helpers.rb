  ...
  module Helpers
    ...
    def ipv6_rule?(new_resource)
      if (new_resource.source && IPAddr.new(new_resource.source).ipv6?) ||
         ...
         new_resource.protocol.to_s =~ /ipv6/ ||
         new_resource.protocol.to_s =~ /icmpv6/
        true
      else
        false
      end
    ...
    def default_description(new_resource)
      new_resource.description ||
        "Generated by chef from #{cookbook_name}[#{recipe_name}] by #{declared_resource(new_resource)}"
    end

    def declared_resource(new_resource)
      resource_type = new_resource.respond_to?(:declared_type) && new_resource.declared_type ? new_resource.declared_type : new_resource.resource_name

      "#{resource_type}[#{new_resource.name}]"
    end
  end
end
