describe firewalld do
  ...
  its('default_zone') { should eq 'zhome' }
  it { should have_rule_enabled('rule family="ipv4" port port="443" protocol="tcp" accept', 'zhome') }
...
describe command('firewall-cmd --permanent --helper=minimal-helper --get-description') do
  its(:stdout) { should cmp "Generated by chef from test[firewalld] by firewall_helper[minimal-helper]\n" }
end
...
supports_policy_disable = Gem::Version.new(firewalld_version) >= Gem::Version.new('2.4.0')
...
describe command('firewall-cmd --info-policy=ptest') do
  ...
  EOF
  ptest_config.sub!(/^  priority:/, "  disable: no\n  priority:") if supports_policy_disable
...
describe command('firewall-cmd --info-policy=pminimal') do
  pminimal = <<~EOF
    pminimal#{' (active)' unless supports_runtime_active_only}
      ...
      rich rules:#{' '}
    EOF
  pminimal.sub!(/^  priority:/, "  disable: no\n  priority:") if supports_policy_disable
...
describe command('firewall-cmd --info-zone=zhome') do
  expected_config = <<~EOF.gsub(/(\n[ ]{2}){2,}/, "\n  ")
    zhome (#{'default, ' if supports_runtime_active_only}active)
      target: default
